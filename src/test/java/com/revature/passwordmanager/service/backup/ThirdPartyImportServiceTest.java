...
class ThirdPartyImportServiceTest {
  ...
  void importFromThirdParty_Success() throws Exception {
    ...
    when(encryptionService.decodeKey(any())).thenReturn(testKey);
    ...
    when(mockImporter.parse(eq("csv_data"), eq(testUser), eq(testKey), eq(encryptionService)))
    ...
    verify(vaultService, times(1)).bulkInsert(eq("testuser"), eq(parsedEntries));
  }

  @Test
  void importFromThirdParty_UnsupportedSource() throws Exception {
    // Arrange
    ThirdPartyImportRequest request = new ThirdPartyImportRequest();

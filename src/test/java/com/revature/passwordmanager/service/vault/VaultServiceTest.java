...
class VaultServiceTest {
  ...
  void accessSensitiveEntry_ShouldThrowException_WhenPasswordInvalid() {
    ...
    when(authenticationManager.authenticate(any())).thenThrow(new org.springframework.security.authentication.BadCredentialsException("Invalid master password"));
  ...
  void getPassword_ShouldReturnPassword_WhenNotSensitive() {
    ...
    assertEquals("encryptedPass", password);
    verify(auditLogService).logAction(eq("testuser"), any(), anyString());
  }
